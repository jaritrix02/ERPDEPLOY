...
const blank = { itemName:'', itemType:'FINISHED', unitId:'', gstId:'', description:'', minStockLevel:0 }
...
export default function FinishedGoods() {
  ...
  const load = () => Promise.all([
    ...
    setItems(i.data.data.filter(it => it.itemType === 'FINISHED')); 
  ...
  const save = async () => {
    try {
      ...
      toast.success(editing ? 'Finished Good updated' : 'Finished Good added')
      setModal(false); load()
  ...
  return (
    <div className="text-black dark:text-white pb-10">
      <PageHeader title="Finished Goods Master" subtitle={`Managing ${items.length} final products`}
        actions={<>
          <div className="relative" ref={itemDropdownRef}>
            <input className="input-field w-64 font-bold rounded-xl" placeholder="Search Products..." value={search} onFocus={() => setShowItemDropdown(true)} onChange={e => { setSearch(e.target.value); setShowItemDropdown(true); }} />
            {showItemDropdown && (
                <div className="absolute z-50 w-full mt-1 bg-white dark:bg-[#1e293b] border border-black/10 dark:border-white/10 rounded-xl shadow-2xl max-h-56 overflow-y-auto">
                    {items.filter(i => i.itemName.toLowerCase().includes(search.toLowerCase()) || i.itemCode.toLowerCase().includes(search.toLowerCase())).map(i => (
                        <div key={i.id} onClick={() => { setSearch(i.itemName); setShowItemDropdown(false); }} className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer border-b border-slate-100 dark:border-slate-800 last:border-0 group text-left">
                            ...
                            <p className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase">FINISHED GOOD</p>
                        </div>
                    ))}
                </div>
            )}
          </div>
          <button className="btn-primary" onClick={openAdd}>+ Add FG</button>
        </>} />
        ...
        <div className="card table-container border-b-4 border-black dark:border-white shadow-2xl">
          <table className="w-full text-left">
            <thead className="table-head bg-black dark:bg-slate-900">
              <tr>
                <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">FG Code</th>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Update Finished Good' : 'Register New FG'} size="xl">
        <div className="space-y-6 text-black dark:text-white p-2">
          <FormRow cols={2}>
            <div>
                <label className="label">Item Type</label>
                <input className="input-field bg-slate-100 dark:bg-slate-800/50 font-bold" readOnly value="FINISHED" />
            </div>
            <div>
              <label className="label">System Code</label>
              <input className="input-field bg-slate-50 dark:bg-slate-800/50 font-mono font-bold" readOnly value={form.itemCode || 'Auto: FG-XXX'} />
            </div>
          </FormRow>
          <div><label className="label">Product Name *</label><input className="input-field font-bold uppercase" value={form.itemName} onChange={e => set('itemName',e.target.value)} /></div>
          <FormRow cols={2}>
            ...
            <div className="relative" ref={gstDropdownRef}>
                <label className="label">GST Config</label>
                <input className="input-field font-bold uppercase rounded-xl" value={gstSearch} onFocus={()=>setShowGstDropdown(true)} onChange={e=>{setGstSearch(e.target.value);setShowGstDropdown(true)}} />
            </div>
          </FormRow>
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800 mt-6">
            ...
            <button className="btn-primary px-12" onClick={save}>Save Product</button>
          </div>
        </div>
      </Modal>
    </div>
