...
router.post('/', async (req,res) => {
  try {
    const modelMap = {units:'unitOfMeasurement',gst:'gSTRate',terms:'termsAndConditions',vendors:'vendor',stores:'store'};
    const d = await prisma[modelMap['stores']].create({data:req.body});
...
router.put('/:id', async (req,res) => {
  try {
    const modelMap = {units:'unitOfMeasurement',gst:'gSTRate',terms:'termsAndConditions',vendors:'vendor',stores:'store'};
    const d = await prisma[modelMap['stores']].update({where:{id:req.params.id},data:req.body});
    res.json({success:true,data:d});
  } catch(e){ res.status(500).json({success:false,message:e.message}); }
});
router.delete('/:id', async (req,res) => {
  try {
    const modelMap = {units:'unitOfMeasurement',gst:'gSTRate',terms:'termsAndConditions',vendors:'vendor',stores:'store'};
    await prisma[modelMap['stores']].delete({where:{id:req.params.id}});
